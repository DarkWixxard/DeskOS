...
if [ ! -d "${REPO_DIR}/apps/backend/dist" ] || [ ! -d "${REPO_DIR}/apps/frontend/.next" ]; then
  ...
  sudo -u "$TARGET_USER" bash -lc "cd '${REPO_DIR}' && npm install && npm run build"
else
  echo "==> Existing build found (skipping). Re-run 'npm run build' after code changes."
fi
...
✅ Done!
  ...
  Dashboard:  http://localhost:3000   (backend API on :3001)
