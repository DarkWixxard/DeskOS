...
// Holographic on/off toggle switch (track + sliding knob). Controlled.
  ...
  return (
    <button
      ...
      className={clsx(
        ...
        checked ? 'border-success/60 bg-success/20' : 'border-accent/30 bg-darker/60'
      )}
    >
      <span
        className={clsx(
          'inline-block h-3.5 w-3.5 rounded-full transition-transform',
          checked ? 'translate-x-[18px] bg-success shadow-glow-sm' : 'translate-x-[3px] bg-accent/40'
        )}
      />
    </button>
  );
